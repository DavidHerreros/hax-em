    ...
    def __call__(self, **kwargs):
        ...
        if "projection_parameters" in kwargs.keys():
            ...
            if "ctfDefocusU" in projection_parameters.keys():
                ...
                preExposure = projection_parameters.get("preExposure", None)
                ctfScaleFactor = projection_parameters.get("ctfScaleFactor", None)
...
def adjust_weights_to_images(model, md_path, mmap_output_dir, sr, batch_size=256, learning_rate=0.01, num_epochs=3,
    ...
    """
    # Prepare metadata. Tilt-series metadata has to be opened in tomo mode so the generator
    # expands the sub-tomogram labels the same way the networks do.
