...
def _write_one_shard_array_record(path, image_indices, getImage_fn, tomo=None, dtype=np.float16):
    """
    Worker: loads images for a shard and writes a single ArrayRecord file.
    Inyecta el ID del subtomo si está disponible para tener 3 ítems.
    """
    ...
    if tomo is not None:
        subtomo_label_struct = struct.Struct("<I")
    ...
    try:
        for i, (img, label) in enumerate(zip(imgs, image_indices)):
            raw = img.tobytes()
            ...
            # Empaquetamos el label, (opcionalmente el tomo), el header y la imagen
            if tomo is None:
                record_bytes = label_struct.pack(int(label)) + header + raw
            else:
                record_bytes = label_struct.pack(int(label)) + subtomo_label_struct.pack(int(tomo[i])) + header + raw
...
def parse_and_decompress(record_bytes, sinusoid_table=None):
    """
    Descomprime devolviendo (Imagen, Sinusoide, Índice).
    """
    cursor = 0
    label = struct.unpack('<I', record_bytes[cursor:cursor + 4])[0]
    cursor += 4

    if sinusoid_table is not None:
        subtomo_idx = struct.unpack('<I', record_bytes[cursor:cursor + 4])[0]
        cursor += 4
        sinusoid = sinusoid_table[subtomo_idx - 1]
    ...
    if sinusoid_table is not None:
        return image.astype(np.float32, copy=False)[..., None], sinusoid, label
    else:
        return image.astype(np.float32, copy=False)[..., None], None, label
...
class MetaDataGenerator:
    def __init__(self, file, mode=None, d_hid=100):
        self.file = file
        ...
        if mode == "tomo" or self.md.isMetaDataLabel("subtomo_labels"):
            ...
            self.sinusoid_table = get_sinusoid_encoding_table(np.amax(unique_labels), d_hid)
        else:
            self.sinusoid_table = None
    ...
    def __getitem__(self, idx):
        if self.mode == "tomo":
            sub_label_idx = int(self.md[idx, "subtomo_labels"]) - 1
            return self.md.getMetaDataImage(idx)[..., None], self.sinusoid_table[sub_label_idx], idx
        else:
            return self.md.getMetaDataImage(idx)[..., None], np.array([-1.0], dtype=np.float32), idx
    ...
    def load_images_to_array_record(self, mmap_output_dir=None, images_order=None, multiple_files=True,
                                    shard_size=10000,
        ...
        if not existing:
            ...
            if not multiple_files:
                ...
                try:
                    ...
                    if self.mode == "tomo":
                        subtomo_label_struct = struct.Struct("<I")
                        tomo_labels = self.md.getMetaDataColumns("subtomo_labels").astype(int)
                    else:
                        subtomo_label_struct = None
                    ...
                    with tqdm(total=len(images_order), file=sys.stdout, ascii=" >=", colour="green") as pbar:
                        for i in range(0, len(images_order), batch_reading_size):
                            ...
                            if self.mode == "tomo":
                                subtomo_labels_batch = tomo_labels[idxs]
                                for img, label, sub_label in zip(imgs, idxs, subtomo_labels_batch):
                                    record_bytes = label_struct.pack(int(label)) + subtomo_label_struct.pack(
                                        int(sub_label)) + header + img.tobytes()
                                    writer.write(record_bytes)
                                    pbar.update(1)
                            else:
                ...
                for start in range(0, n, shard_size):
                    ...
                    path = os.path.join(mmap_output_dir, shard_name)
                    t_labels = self.md.getMetaDataColumns("subtomo_labels")[idxs].astype(
                        int) if self.mode == "tomo" else None
                    shards.append((path, idxs, t_labels))
                    shard_idx += 1

                with tqdm(total=n, file=sys.stdout, ascii=" >=", colour="green") as pbar:
                    with ProcessPoolExecutor(max_workers=num_workers, mp_context=get_context('forkserver')) as ex:
                        futures = [
                            ex.submit(_write_one_shard_array_record, path, idxs, self.md.getMetaDataImage, t_labels,
                                      precision)
                            for (path, idxs, t_labels) in shards
                        ]

                        for fut, (_, idxs, _) in zip(as_completed(futures), shards):
                            fut.result()
    ...
    def return_tf_dataset(self, preShuffle=False, shuffle=True, prefetch=-1, batch_size=8, mmap=True,
                          mmap_output_dir=None, split_fraction=None):
        ...
        with tf.device("/CPU:0"):
            ...
            if mmap:
                ...
                def _load_image(idx):
                    ...
                    if self.mode == "tomo":
                        ...
                        return image, np.array([-1.0], dtype=np.float32), idx

                def map_fn(i):
                    (image, subtomo_labels, idx) = tf.numpy_function(_load_image, [i],
                    ...
                    if self.mode == "tomo":
                        ...
                        subtomo_labels.set_shape([None, 1])
                ...
                if self.mode == "tomo":
                    ...
                    empty_labels = np.full((len(file_idx), 1), -1.0, dtype=np.float32)
                    dataset = tf.data.Dataset.from_tensor_slices(((images, empty_labels), file_idx))
            ...
            if split_fraction is None:
                ...
                if mmap:
                    ...
                    dataset_validation = dataset_validation.map(map_fn, num_parallel_calls=tf.data.AUTOTUNE,
                                                                deterministic=False)

                return tfds.as_numpy(dataset.prefetch(prefetch)), tfds.as_numpy(
                    dataset_train.prefetch(prefetch)), tfds.as_numpy(dataset_validation.prefetch(prefetch))
    ...
    def return_torch_dataset(self, shuffle=True, preShuffle=True, batch_size=8, mmap=True, mmap_output_dir=None):
        ...
        if self.mode == "tomo":
            subtomo_labels = self.sinusoid_table[self.md[file_idx, "subtomo_labels"].astype(int) - 1]
            dataset = TensorDataset(torch.from_numpy(images), torch.from_numpy(subtomo_labels),
                                    torch.from_numpy(file_idx))
        else:
            dataset = TensorDataset(torch.from_numpy(images), torch.full((len(file_idx), 1), -1.0, dtype=torch.float32),
                                    torch.from_numpy(file_idx))

        def numpy_collate(batch):
            return tree_map(np.asarray, default_collate(batch))

        return DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, num_workers=16, collate_fn=numpy_collate,
                          persistent_workers=True,
                          pin_memory=True)

    def prepare_grain_array_record(self, mmap_output_dir=None, preShuffle=False, num_workers=16, precision=np.float16,
                                   group_size=1,
                                   shard_size=10000):
        ...
        self.load_images_to_array_record(mmap_output_dir=self.mmap_output_dir, images_order=file_idx,
                                         num_workers=num_workers,
                                         precision=precision, group_size=group_size, shard_size=shard_size)

    def prepare_grain_mmap(self, mmap_output_dir=None, preShuffle=False, num_workers=16, shard_size=10000,
        ...
        self.load_images_to_mmap(mmap_output_dir=self.mmap_output_dir, images_order=images_order,
                                 num_workers=num_workers,
    ...
    def return_grain_dataset(self, shuffle="global", batch_size=8, num_epochs=1, num_threads=1, num_workers=16,
        ...
        if self.grain_dataset_type == "ArrayRecord":
            ...
            if split_fraction is not None:
                ...
                sources_train = ArrayRecordDataSource(shard_files[:split_point],
                                                      reader_options={"index_storage_option": "in_memory"})
                sources_val = ArrayRecordDataSource(shard_files[split_point:],
                                                    reader_options={"index_storage_option": "in_memory"})
            ...
            class LazyNinjaGrainSource:
                def __init__(self, shard_paths, md, sinusoid_table):
                ...
                def __getitem__(self, idx):
                    ...
                    local_idx = idx if shard_idx == 0 else idx - self.cumulative_indices[shard_idx - 1]
                    shard = self._get_shard(shard_idx)

                    img = shard[local_idx][..., None].astype(np.float32)
                    sin_val = self.sinusoid_table[
                        int(self.md[idx, "subtomo_labels"]) - 1] if self.sinusoid_table is not None else np.array(
                        [-1.0], dtype=np.float32)

                    return img, sin_val, idx
            ...
            if split_fraction is not None:
                ...
                sources_train = LazyNinjaGrainSource(shard_paths, self.md, self.sinusoid_table)
                dataset_train = grain.MapDataset.source(sources_train)
            ...
            class NumpyDataSource:
                def __init__(self, data, labels, md, sinusoid_table):
                    self._data = data
                    self._labels = labels
                    self._md = md
                    self._sinusoid_table = sinusoid_table

                def __len__(self):
                    return len(self._data)

                def __getitem__(self, idx):
                    global_idx = self._labels[idx]
                    sin_val = self._sinusoid_table[int(
                        self._md[global_idx, "subtomo_labels"]) - 1] if self._sinusoid_table is not None else np.array(
                        [-1.0], dtype=np.float32)
                    return self._data[idx], sin_val, global_idx
            ...
            if split_fraction is not None:
                ...
                sources_train = NumpyDataSource(images, labels, self.md, self.sinusoid_table)
                dataset_train = grain.MapDataset.source(sources_train)
        ...
        if shuffle == "global":
            ...
            if self.grain_dataset_type == "ArrayRecord":
                dataset_train = dataset_train.map(lambda x: parse_and_decompress(x, self.sinusoid_table))
                if split_fraction is not None:
                    dataset_val = dataset_val.map(lambda x: parse_and_decompress(x, self.sinusoid_table))
            ...
            if self.grain_dataset_type == "ArrayRecord":
                class ParseAndDecompress(grain.transforms.Map):
                    def __init__(self, sinusoid_table):
                        self.sinusoid_table = sinusoid_table

                    def map(self, x):
                        return parse_and_decompress(x, self.sinusoid_table)

                operations = [ParseAndDecompress(self.sinusoid_table)]
...
def extract_columns(md, hasCTF=None, isTomo=None):
    ...
    columns = {}
    columns["euler_angles"] = jnp.array(
        md.getMetaDataColumns(["angleRot", "angleTilt", "anglePsi"]).astype(jnp.float32))
    ...
    if hasCTF:
        ...
        columns["ctfSphericalAberration"] = jnp.array(
            md.getMetaDataColumns("ctfSphericalAberration").astype(jnp.float32))
...
def get_sinusoid_encoding_table(n_position, d_hid, padding_idx=None):
    ...
    # Creamos la tabla hasta n_position para evitar out_of_bounds (ya que labels empiezan en 1)
